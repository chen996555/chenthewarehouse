...
| 方式 | 依赖 | 失效点 | 代表 |
|---|---|---|---|
| DOM 抓取 | CSS class / DOM 结构 | 前端改版 | moka |
| 纯 HTTP 接口 | URL + 字段名 | 字段改名 / 接口迁移 | jd/mt/tx/ali/ctrip/zhiye |
| 页面上下文 fetch | 接口字段 + 页面 cookie | 认证升级 | hotjob/bili/ant |
| 浏览器拦截响应 | 接口 URL + 响应字段 | 响应字段改名（签名自动跟页面走）| byte |

### 5. 浏览器型适配器一次会话循环多关键词
**决策**：byte/hotjob/moka/bili/ant 接受关键词数组，一次浏览器会话循环搜，不重复开浏览器。
**why**：开浏览器是最大耗时（6-10s/次），多关键词开多次是浪费。改成单会话循环后扫描从 4-6 分钟降到 ~4 分钟。
...
**已知局限**：健康基线对比「上次扫描」没意识到画像换人，换人后岗位数变化会误报「疑似」。待改进：基线记画像 owner，换人时重置。
...
2. **适配器逆向**：16 类 ATS 系统（北森 zhiye/字节/北森微招聘/Moka/京东/美团/腾讯/阿里/拼多多/快手/小红书/B站/蚂蚁/米哈游/携程/网易），逐个逆向接口 + 搜索字段 + 认证。
...
**当前**：16 类适配器全关键词搜索；稳定性机制五层；缓存画像隔离；画像污染已修；三份简历验证通用性。**半自动投递闭环跑通**（apply.js：上传简历/点解析/补漏 + 自定义下拉精确选 + 简历库多简历匹配）。

**下一步**（按优先级）：
1. 投递记录 + 状态跟踪（待投→已投→有回复→面试→Offer 状态机，确定性非 LLM）
2. 看板加「候选人」维度（换人后待投岗位混在一起）
3. 健康基线识别画像切换（换人重置基线）
4. 反馈闭环（投递结果回校准打分器）
